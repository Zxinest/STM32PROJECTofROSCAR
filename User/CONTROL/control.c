...
int TIMING_TIM_IRQHandler(void)
{
	...
	{			
		...
		if(++Lidar_flag_count==20) Lidar_Success_Receive_flag=0,Lidar_flag_count=0; //50ms清零雷达接收到数据标志位
...
void Lidar_Follow(void)
{
	...
	{
		if(++data_count == 30)		//连续30次采集到的值(150ms后)和上次的比大于10度，此时才是认为是有效值
		{
			data_count = 0;
			last_angle = angle;
		}
	...
	else
	{
		...
		else//其余车型
		{
			...
			{
				Move_Z = -0.1f*last_angle;  //角度差距过大直接快速转向
	...
	if(Move_X==0)
		Move_Z = target_limit_float(Move_Z,-Pi/2,Pi/2);   //限幅
	else
